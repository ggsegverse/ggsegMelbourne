...
functional connectivity gradients, at four nested scales. Scale I splits the
subcortex into 8 structures per hemisphere; each finer scale subdivides the
scale above, down to 27 structures per hemisphere at scale IV.

Each scale ships in a 3 Tesla and a 7 Tesla variant; the 7T parcellations are
derived from higher resolution data and are slightly finer at the same scale.
...
subdivisions in shades of that hue, so a structure keeps its colour across
scales and hemispheres.
...
> gradients." *Nature Neuroscience*, 23, 1421-1432.
...
Please note that the ggsegTian project is released with a
