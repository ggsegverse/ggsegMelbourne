...
one parcel of the scale above, so `tian_s1()` regions are unions of
`tian_s2()` regions and so on. Colour follows that hierarchy, one hue
...
- `tian_s1()`: Scale I, 8 structures per hemisphere: hippocampus,
  amygdala, nucleus accumbens, pallidum, putamen, caudate, and the
  thalamus split into an anterior and a posterior division.

- `tian_s2()`: Scale II, 16 structures per hemisphere. Each scale I
  structure splits in two, along an anterior-posterior, medial-lateral
  or core-shell axis.

- `tian_s3()`: Scale III, 25 structures per hemisphere.

- `tian_s4()`: Scale IV, 27 structures per hemisphere, the finest
  parcellation published for 3 Tesla data.

- `tian_s1_7t()`: Scale I from 7 Tesla data, 8 structures per
  hemisphere.

- `tian_s2_7t()`: Scale II from 7 Tesla data, 17 structures per
  hemisphere.

- `tian_s3_7t()`: Scale III from 7 Tesla data, 27 structures per
  hemisphere.

- `tian_s4_7t()`: Scale IV from 7 Tesla data, 31 structures per
  hemisphere.
